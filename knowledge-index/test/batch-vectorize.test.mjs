...
test('S-03: batchVectorize 部分失败', () => {
  ...
  const r = batchVectorize(entries, 'mock', { memBinPath: MOCK_MEM });
  delete process.env.MOCK_FAIL_PATHS;
...
test('S-03: batchVectorize 空数组', () => {
  const r = batchVectorize([], 'mock', { memBinPath: MOCK_MEM });
  assert.equal(r.ok.size, 0);
  assert.equal(r.errors.length, 0);
});

test('S-03: deleteMemory 成功', () => {
  const r = deleteMemory('abc-123', { memBinPath: MOCK_MEM });
  assert.equal(r.ok, true);
});
