...
| 评分机制 | 使用频率评分 | 5分钟内多次调用不上调，5分钟外上调，1天前下调，更准确反映使用频率 | 简单+1计数：无法区分使用频率，可能被临时高频调用误导 |
| 热门索引机制 | 优先返回热门索引 | Agent 首次查询时返回热门索引，提高查询效率 | 全量返回：信息过载，Agent 需要筛选 |
...
| 冷热分区 | 三区机制（热/常温/冷） | 根据评分进行分区，分区有数量限制，超出后转移或删除 | 单一分区：无法区分使用频率，缓存效率低 |
...
| 评分机制算法不准确 | 热门索引判断不准确，影响查询效率 | 评分机制单独设计，后续继续讨论优化算法 |
| 数据备份失败 | 知识库丢失后无法恢复 | 定期备份机制，备份失败时告警 |
| 归档数据过多 | 存储空间占用过大 | 归档数据定期清理，保留最近 N 个月的数据 |

### 14.2 待定问题（Open Questions）

- [ ] **[待讨论]** 评分机制的具体算法需要进一步讨论，当前设计为：5分钟内多次调用不上调，5分钟外上调，1天前下调
